import React from "react";
import Reb from "../src/images/retained-header (2).png";
import I1 from "../src/images/dental-implants (1).png";
import I2 from "../src/images/experienced-clinicians.png";
import I3 from "../src/images/lifetime.png";
import I4 from "../src/images/life-changing-results.png";
import I5 from "../src/images/exceptional-aftercare.png";
import Ds from "../src/images/shutterstock_1494042974-1.jpg";
import Ta from "../src/images/talk@2x.png";
import Xr from "../src/images/x ray.png";
import Nr from "../src/images/fitted implant.png";
import N2 from "../src/images/healing.png";
import Show1 from "../src/images/patient-dm-1 (1).jpg";
import Show2 from "../src/images/patient-dm-1 (2).jpg";
import Show3 from "../src/images/patient-dm-1.jpg";
import Show4 from "../src/images/patient-dm-2 (1).jpg";
import Show5 from "../src/images/patient-dm-3.jpg";
import Imrd1 from "../src/images/imrd1.jpg";
import Imrd2 from "../src/images/imrd2.jpg";
import Imrd3 from "../src/images/imrd3.png";
import St1 from "../src/images/Single-tooth.jpg";
import St2 from "../src/images/multiple-implants@2x.jpg";
import St3 from "../src/images/implants-retained.jpg";
import All1 from "../src/images/All-on-6@2x-copy (1).jpg";
import All2 from "../src/images/all-on-4@2x.jpg";
import All3 from "../src/images/teeth-in-a-day@2x.jpg";
import Footer from './Footer';



const Implantretaineddentures =() => {
      
return (

<>

<section id = "pedic" className ="d-flex align-items-center easy mt-5" >  
       <div className ="container">
<div className ='row'>
  <div className ="col-10 mx-auto">
    <div className ="row">                                
  
   <div className ="col-md-12 pt-5 pt-lg-0 order-2 order-lg-1 d-flex align-items-center "> 
   <div className ='row'>
    <h1 className="hard"> Implant retained denture<br/> dental implants in Northampton </h1>
  <div className="row">
   <h2 className =" hard ">  The cost-effective, most permanent & lifelike <br/>solution to replacing a single or both <br/>arches of missing teeth (Starts from£83) </h2>

 <img src={Reb} className="img-fluid  " alt=" surgery"style={{height:"15.83rem",width:"29.83rem"}}/>
</div>
</div>
      </div>
     
 </div>
</div>
</div>

</div>

       </section >

       <div className="cure">
       <div className="container mt-5  ">
 
 <div className ="row">
  
   <div className="col-lg-2 col-md-2 col-4 mb-5 gy-5">
   <div class="card text-center"style={{height:"8.83rem",width:"8.83rem"}}>
  
  <img src={I1} className="img-fluid border-radius p-4 perfect" alt=" surgery"style={{height:"6.83rem",width:"6.83rem"}}/>
  <div className="card-body  ">World Leading Implant</div>
  </div>
   </div>
   <div className=" hide col-lg-2 col-md-2 col-4 mb-5 gy-5 mid1">
   <div className="card text-center"style={{height:"8.83rem",width:"8.83rem"}}>
  
  <img src={I2} className="img-fluid border-radius p-4 perfect" alt=" surgery"style={{height:"5.83rem",width:"6.83rem"}}/>
  <div className="card-body beautiful">Highly experienced clinicians</div>
 
</div>
   </div>
   <div className="col-lg-2 col-md-2 col-4 mb-5 gy-5">
   <div className="card text-center add2"style={{width:"8.83rem",height:"8.83rem"}}>

  <img src={I3} className="img-fluid border-radius p-4 perfect" alt=" surgery"style={{height:"5.83rem",width:"6.83rem"}}/>
  <div className="card-body beautiful  ">Lifetime Warranty</div>
  
</div>
   </div>
   <div className="hide col-lg-2 col-md-2 col-4 mb-5 gy-5">
   <div className="card text-center add2"style={{width:"8.83rem",height:"8.83rem"}}>

  <img src={I4} className="img-fluid border-radius p-4 perfect" alt=" surgery"style={{height:"5.83rem",width:"6.83rem"}}/>
  <div className="card-body beautiful "> Life Changing Results</div>
  
</div>
   </div>

  
   <div className="hide col-lg-2 col-md-2 col-4 mb-5 gy-5">
   <div className="card text-center add3"style={{width:"8.83rem",height:"8.83rem"}}>
  
  <img src={I5} className="img-fluid border-radius p-4" alt=" surgery"style={{height:"5.83rem",width:"8.83rem"}}/>
  <div className="card-body beautiful"><br/>Exceptional aftercare and service as standard</div>
 
   </div>
   </div>

 </div>
 </div>
</div>
<section id="enlarge">
  <div className="cure">

  </div>
</section>


<section className="reviews-ib container-fluid " id='reviews-ib'>
  <div className="py-5 text-center beautiful">
    <div>

<div className ='row'>
  <div className ="col-10 mx-auto text-center brace">
    <div className ="row gy-4" >
    <div className="col-md-4 col-10 mx-auto">
   <h5 className="brace-1">I had my consultation with Dr Buddha. He was by far the most professional compared to the other clinics I went to for consultations this week. His colleague Lisa was very friendly and helpful.<hr/>

Sue Green</h5>
    </div>
     <div className="col-md-4 col-10 mx-auto">
   <h5 className="brace-1" >So I went to see myclinique this week for a Invisalign consultation and I was very impressed, I saw a lady called Lisa and a man called Mark, I told them what a great consultation it was because I was informed of lot's of options and were given lots of information about Invisalign that other clinics never told me before.

   <hr/>Natalia Marjovic</h5>
    </div>
     <div className="col-md-4 col-10 mx-auto">
  <h5 className="brace-1" >MyClinique is the best clinic in Northampton my daughter got her Invisalign here with the gentleman and I get my anti-wrinkle injections here.

  <hr/>Marianne Donova</h5>
    </div>
    
    </div>
    </div>
    </div>
</div>

</div>


</section>

<div className="notreal container-fluid mb-5 reviews-ib d-flex justify-content-center">
   <div className ="row d-flex justify-content-center">
  
   <div className="real-1 col-lg-4 col-md-4 col-12">
   <div className="card text-center"style={{width:"18.83rem"}}>
  
  <div className="card-body"> Secure</div>
  <div className="card-footer bg-primary">Implant retained dentures provide a more secure solution when compared to regular dentures


   </div>
</div>
   </div>

   <div className="real-1 col-lg-4 col-md-4 col-12">
   <div className="card text-center"style={{width:"18.83rem"}}>
  
  <div className="card-body">Better appearance</div>
  <div className="card-footer bg-primary">Implant retained dentures look fare more realistic than other dentures


   </div>
</div>
   </div>
   <div className="real-1 col-lg-4 col-md-4 col-12">
   <div className="card text-center"style={{width:"18.83rem"}}>

  <div className="card-body">Long-lasting</div>
  <div className="card-footer bg-primary">Implant retained dentures last significantly longer than traditional dentures
 </div>
</div>
  
  </div> 
</div>
 </div>

 <section id="bismillah">
 <div className="col-lg-6 col-md-6 cell ">
   <div className="row">
<p className="pl-5 text-center">Implant retained dentures provide better performance than regular dentures at a lower cost than other forms of dental implants.

Implant retained dentures often require only two dental implants. This makes them even simpler and more affordable than All-on-4 dental implants. Implant retained dentures cannot provide the same “real-teeth” feeling as implant supported dentures, but they look more realistic than traditional removable dentures.

They not only look better, they function better too. What’s more, they help to reduce the bone loss which is inevitable when you use dentures without dental implants.

Implant retained dentures use much the same materials as implant-supported dentures, as the dental implants themselves are generally made of titanium coated in acrylic. This helps to ensure longevity. The dentures are typically made of zirconia (for strength) and porcelain (for aesthetic).</p>
                                                            
    <div className ="mt-3 place">
    
    <a href= "http://localhost:3000/Contactus" className ="mt-5 swagButton">Discuss how you want to proceed </a>                                                             


<div className="row">
  
   <img src={Imrd1} className="pl-5 img-fluid side pb-5" alt=" surgery"style={{width:"25.83rem"}}/>
   <div className="row">
   </div>
   </div>
  
   </div>
   </div>
   </div>
</section>
<section  id="agee" className="mt-5 reviews-ib">
<h3 className=" text-center beautiful"><br/>Pros & cons</h3>
<p className="hide text-center beautiful " >Here is a quick guide to the pros and cons of implant retained dentures</p>
<div className="iskob exactly mt-2 d-flex justify-content-center">
  <div className="column " >
    <h4 className="paright text-center beautiful "><br/>Quicker and simpler procedure than<br/> other implant procedures</h4>
    <p className="paright  text-center beautiful" >Implant retained dentures generally take far<br/> less time to complete than other teeth replacement options</p>
    <h4 className="paright  text-center beautiful" >Look much more realistic than regular dentures</h4>
    <p className="paright  text-center beautiful">Implant retained dentures look <br/>as close to real teeth as possible, far surpassing<br/> the appearance of traditional dentures</p>
    <h4 className="paright  text-center beautiful" > You will be able to speak as though you had real teeth</h4>
    <p className=" paright text-center beautiful" >Unlike removeable dentures, you won't have<br/> to worry about your implant retained denture coming loose <br/>or affecting your speech</p>
    <h4 className="paright  text-center beautiful" >You will be able to eat most foods</h4>
    <p className="paright  text-center beautiful">An implant retained denture will give you the freedom <br/>to return to eating most foods, unlike traditional<br/> dentures which restrict your diet</p>
    <h4 className="paright  text-center beautiful" >Implant retained dentures are very cost-effective <br/>compared to other dental implants</h4>
    <p className="paright  text-center beautiful">An implant retained denture is a much more affordable<br/> way to replace several missing teeth than other options</p>


  </div>
  <div className="column hide" >
    <h4 className="line-2 text-center beautiful" ><br/><br/>Very hard and/or sticky foods will probably still be off-limits</h4>
    <p className="line-2 text-center beautiful " >Your dentist will advise you on which foods<br/> you'll need to be careful with after having your implant<br/> retained denture fitted</p>
    <h4 className="line-2 text-center beautiful" >You’ll still need to go through the healing process associated with dental implants</h4>
    <p className="line-2 text-center beautiful ">As you will have implants fitted into your jaw,<br/> you will have healing times to contend <br/>with after your procedure</p>
    <h4 className="line-2 text-center beautiful" >More expensive than regular dentures</h4>
    <p className="line-2 text-center beautiful ">As they look and function better, <br/>implant retained dentures are more<br/> expensive than other dentures</p>

  </div>
</div>
 </section>  
 <section id="enlarge2">
  <div className="cure maxm">

  </div>
</section>

<div id="enlarge0"></div>
 <h3 className="brace text-center mt-5 beautiful  ">Multiple Dental implant procedure     </h3>
 

<div className="container-fluid mt-5  d-flex justify-content-center ">
  
   <div className ="row">
   <div className="col-lg-6 col-md-6 col-12  bg-light">
   <div class="card text-center"style={{width:"30.83rem",height:"25.83rem"}}>
  
  <img src={Ta} className="img-fluid border-radius p-4 function" alt=" surgery"style={{width:"6.83rem",height:"6.83rem"}}/>
  <div class="card-body">Consultation</div>
  <div class="card-footer "> At the consultation, your dentist will check if you can have the multiple dental implants procedure. They’ll explain how it works and what alternatives you have. and you will be able to ask whatever questions you like. By the end of the consultation, you should be able to make an informed decision on whether or not you want to go ahead with the multiple dental implants procedure..
</div>
</div>
   </div>
   <div className="col-lg-6 col-md-6 col-12 bg-light ">
   <div class="card text-center"style={{width:"30.83rem",height:"25.83rem"}}>
  
  <img src={Xr} className="img-fluid border-radius p-4 function" alt=" surgery"style={{width:"6.83rem",height:"6.83rem"}}/>
  <div class="card-body">Appointment </div>
  <div class="card-footer ">The length of your multiple dental implants appointment will depend on exactly what needs to be done and how many implants are being place. The procedure will, however, require general anaesthetic, so you may wish to leave the entire day free to recover.
    </div>
</div>
   </div>
  </div>
 </div>


 <div className="container-fluid   d-flex justify-content-center ">
   <div className ="row">
   <div className="col-lg-6 col-md-6 col-12 mb-5 bg-light">
   <div class="card text-center"style={{width:"30.83rem",height:"25.83rem"}}>
  
  <img src={Nr} className="img-fluid border-radius p-4 function" alt=" surgery"style={{width:"6.83rem",height:"6.83rem"}}/>
  <div class="card-body">Fitted Implant</div>
  <div class="card-footer bg-light">Your dentist will fit the implant screws into your jaw. These will then have an abutment fitted over them and finished with temporary teeth. Whilst the temporary teeth still look like real life,they are made from a much lighter material to place minimal pressure onto the jaw whilst healing.
</div>
</div>
   </div>
   <div className="col-lg-6 col-md-6 col-12 bg-light ">
   <div class="card text-center"style={{width:"30.83rem",height:"25.83rem"}}>
  
  <img src={N2} className="img-fluid border-radius p-4 function" alt=" surgery"style={{width:"6.83rem",height:"6.83rem"}}/>
  <div class="card-body">Healing</div>
  <div class="card-footer bg-light">You should allow a few days for the initial swelling and bruising to fade. After that, it will take about 6 months for your mouth to heal completely. Once the healing process is complete, you will get your permanent teeth fitted onto the implants. If you look after them properly, you can enjoy your new teeth for many years.
    </div>
</div>
   </div>
  </div>
 </div>

 <section>
      <div className=" sobia mt-5" >
      <h3 className="line=5 beautiful">Dental implant patients</h3>
      <h6 className="text-center"> From A-listers to the general public, everyone gets the same level of care & attention to detail at my clinic. If you think you may want dental implants in Northampton, book your free consultation today </h6>
</div>
</section>

 <div className="container mb-5 mt-5">
  <div className="carousel slide" id ="my-slider" data-ride="carousel">
    <div className="carousel-inner text-center">
     <div className="carousel-item active">
     <img src={Show1} className="img-fluid "alt="slide1" style={{width:"45.83rem",height:"23.83rem"}}/>
     <div className="carousel-caption">
  
    
  </div>
     </div>
     <div className="carousel-item ">
     <img src={Show2} className="img-fluid"alt="slide2"style={{width:"45.83rem",height:"23.83rem"}}/>
     <div className="carousel-caption">
   
  </div>
     </div>
     <div className="carousel-item">
     <img src={Show3} className="img-fluid"alt="slide3"style={{width:"45.83rem",height:"23.83rem"}}/>
     <div className="carousel-caption">
   
  
  </div>
     </div>
     <div className="carousel-item ">
     <img src={Show4} className="img-fluid"alt="slide4"style={{width:"45.83rem",height:"23.83rem"}}/>
     <div className="carousel-caption">
   
  </div>
     </div>
     <div className="carousel-item">
     <img src={Show5} className="img-fluid"alt="slide3"style={{width:"45.83rem",height:"23.83rem"}}/>
     <div className="carousel-caption">
  
  </div>
     </div> 
   
    </div>
</div>
  </div>


  <h3 className="brace text-center beautiful">Are implant retained dentures right for me?<br/><br/><br/><br/></h3>
  
  <section id="bismillah">
 <div className="col-lg-6 col-md-6 cell ">
   <div className="row">
   <div className="leftside">
     <h6  className="brand-name pl-5">Implant retained dentures are perfect for</h6>

<ul >
  <li className="pl-5">Replacing a whole arch of missing teeth</li>
  <li className="pl-5">Providing a cost-effective solution</li>
  <li className="pl-5">Giving better functionality than traditional dentures</li>
 
  <li  className="pl-5 spabove hide">Preserving bone density in the jaw</li>
  

</ul>
</div>

<p className="pl-5 hide">The more implants you need to have fitted into your jaw, the longer the procedure will take to perform. This not only increases the costs, but it can also increase the immediate post-surgery bruising and swelling. For some people, this is reason enough to opt for implant retained dentures over other options such as All-on-6 or Teeth in a Day.</p>

<div className ="mt-3 gbtn">
    
    <a href= "" className ="btn- btn">Book your FREE consultation Now</a>                                                             

<div className="row">
  
   <img src={Imrd2} className="pl-5 pb-5 img-fluid side" alt=" surgery"style={{width:"30.83rem",height:"15.83rem"}}/>
   <div className="row">
   </div>
   </div>
  
   </div>
   </div>
   </div>
</section>
<section id="mark">
<h4 className="text-center beautiful line-3">Implant Retained Denture Prices</h4>
<div className="row">
<div className="col-lg-6 col-md-6  ">
<h4 className="brand-name"> <br/><span className="change-content brand-name proper"></span> (£83)</h4>

   <img src={Imrd3} className="img-fluid all-on2" alt=" surgery"style={{width:"30.83rem",height:"15.83rem"}}/>

</div>
   <div className="col-lg-6 col-md-6 cell ">

<p className="text-align: right abb">Implant retained dentures are generally more expensive than regular dentures. They are, however, more affordable than other solutions<br/> to missing teeth, such as All-on-4 dental implants, and are generally<br/> preferred as an alternative to traditional removable dentures.

What’s<br/> more, you may be eligible for a dental finance plan, allowing you to <br/>spread the cost of your treatment over a longer period of time.

View<br/> our pricing page to see our prices for an implant retained denture in Northampton.</p>
   <div className="row">
   
   </div>
  
   </div>
   </div>
  
</section>
<div className="cure mt-5">
<section className="we-are container"id="product mt-5" >
  <div className="py-2 text-center mb-5">
    <div>
    <h3 className="text-center " > <br/>Dental implants product range
 </h3>

<div className ='row'>
  <div className ="col-10 mx-auto text-center">
    <div className ="row gy-2" >
    <div className="col-md-4 col-10 mx-auto">
    <h3 className="line" >Single Tooth <br/>implants</h3>
      <p> <br/><br/>Single dental implants are used to replace one or missing teeth.</p>
    
    <img src={St1} alt=''className="apply" style={{width:"10.83rem"}}/>
    <h4 className="brand-name"> <br/><span className="change-content brand-name proper"></span> (£29)</h4>
    </div>
     <div className="col-md-4 col-10 mx-auto">
     <h3 className="line" >Multiple Tooth Implants <br/>(3+)</h3>
      <p> Multiple dental implants are used to replace 3 or more missing teeth.They can replace missing teeth that are in the same area of the mouth or area spread out over different areas.</p>
     <img src={St2} alt=''style={{width:"10.83rem"}}/>
     <h4 className="brand-name"> <br/><span className="change-content brand-name proper"></span> (£25)</h4>
    </div>
    <div className="col-md-4 col-10 mx-auto">
      <h3 className="line" >Implant Retained <br/>Denture</h3>
      <p><br/> Implant retained dentures are an alternative option to traditional removable dentures.They are used to replace either one or both arches of missing teeth.</p>
     <img src={St3} alt='' className="apply" style={{width:"12.83rem"}}/>
     <h4 className="brand-name"> <br/><span className="change-content brand-name proper"></span> (£83)</h4>
    </div>
    
    </div>
    </div>
    </div>
</div>

</div>


</section>

<section className="we-are container mt-5" id="product" >
  <div className="py-2 text-center mb-5">
    <div>


<div className ='row'>
  <div className ="col-10 mx-auto text-center">
    <div className ="row gy-2" >
    <div className="col-md-4 col-10 mx-auto">
    <h3 className="line" >All-On-6</h3>
      <p>All-on-6 dental implants can replace missing teeth, with long-term results.Six implants are fitted into the jawbone to anchor the overdenture and provide a secure and real-looking finish.</p>
    
    <img src={All1} alt=''style={{width:"10.83rem"}}/>
    <h4 className="brand-name"> <br/><span className="change-content brand-name proper"></span> (£98)</h4>
    </div>
     <div className="col-md-4 col-10 mx-auto">
     <h3 className="line">All-On-4</h3>
      <p> All-on-4 dental implants are a long term solution to replacing missing teeth.Four implants are fitted to the jaw so that the overdenturecan be fitted to look and function just like real teeth.</p>
     <img src={All2} alt=''style={{width:"10.83rem"}}/>
     <h4 className="brand-name"> <br/><span className="change-content brand-name proper"></span> (£99)</h4>
    </div>
    <div className="col-md-4 col-10 mx-auto">
      <h3 className="line" >Teeth In A Day</h3>
      <p>Teeth in a Day can replace both arches of missing teeth, all in one day.This means you can leave the clinic with your new smile on the same day</p>
     <img src={All3} alt=''style={{width:"12.83rem"}}/>
     <h4 className="brand-name"> <br/><span className="change-content brand-name proper"></span> (£99)</h4>
    </div>
    
    </div>
    </div>
    </div>
</div>

</div>


</section>
</div>

  

<Footer/>
 
 







</>
);

};
export default Implantretaineddentures ;